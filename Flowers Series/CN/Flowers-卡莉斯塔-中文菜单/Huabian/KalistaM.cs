...
namespace Flowers滑板鞋_重生_
{
    class KalistaM
    {
        ...
        public static void KalistaMenu()
        {
            ...
            菜单.SubMenu("Combo").AddItem(new MenuItem("lzp", "使用 Q", true).SetValue(true));
            菜单.SubMenu("Combo").AddItem(new MenuItem("lze", "使用 E", true).SetValue(true));
            菜单.SubMenu("Combo").AddItem(new MenuItem("lzeee", "使用 E 击杀", true).SetValue(true));
            菜单.SubMenu("Combo").AddItem(new MenuItem("lzeeeeee", "最大 E 堆叠", true).SetValue(new Slider(5, 1, 20)));
            菜单.SubMenu("Combo").AddItem(new MenuItem("lzmp", "连招最低蓝量 <=%", true).SetValue(new Slider(50, 0, 100)));

            菜单.AddSubMenu(new Menu("骚扰设置", "Harass"));
            菜单.SubMenu("Harass").AddItem(new MenuItem("srq", "使用 Q", true).SetValue(true));
            菜单.SubMenu("Harass").AddItem(new MenuItem("AutoQ", "自动 骚扰", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            菜单.SubMenu("Harass").AddItem(new MenuItem("srmp", "骚扰最低蓝量 <=%", true).SetValue(new Slider(50, 0, 100)));

            菜单.AddSubMenu(new Menu("清理设置", "Clear"));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qxq", "使用 Q 清线", true).SetValue(false));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qxqqq", "使用 Q 命中小兵数", true).SetValue(new Slider(3, 1, 5)));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qxe", "使用 E 清线", true).SetValue(true));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qxeee", "使用 E 命中小兵数", true).SetValue(new Slider(2, 1, 5)));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qyq", "使用 Q 清野", true).SetValue(false));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qye", "使用 E 清野", true).SetValue(true));
            菜单.SubMenu("Clear").AddItem(new MenuItem("eqiangyeguai", "使用 E 抢野怪", true).SetValue(true));
            菜单.SubMenu("Clear").AddItem(new MenuItem("qxmp", "清理最低蓝量 <=%", true).SetValue(new Slider(60, 0, 100)));
      
            
            
            菜单.AddSubMenu(new Menu("大招设置", "RSetting"));
            菜单.SubMenu("RSetting").AddItem(new MenuItem("Save", "拯救基佬", true).SetValue(true));
            菜单.SubMenu("RSetting").AddItem(new MenuItem("jilaohp", "基佬 Hp <=%", true).SetValue(new Slider(20, 0, 100)));

            菜单.AddSubMenu(new Menu("物品使用", "Item"));
            菜单.SubMenu("Item").AddItem(new MenuItem("UseYUU", "使用 幽梦之灵", true).SetValue(true));
            菜单.SubMenu("Item").AddItem(new MenuItem("UseBRK", "使用 破败王者之刃", true).SetValue(true));
            菜单.SubMenu("Item").AddItem(new MenuItem("UseBC", "使用 比尔沃吉特弯刀", true).SetValue(true));

            菜单.AddSubMenu(new Menu("逃跑设置", "Flee"));
            菜单.SubMenu("Flee").AddItem(new MenuItem("Flee", "逃跑 按键", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));            
         ...
         /*   菜单.AddSubMenu(new Menu("Balista", "BiuBiuBiu"));
            ...
            菜单.SubMenu("Misc").AddItem(new MenuItem("DamageExxx", "自动 E -> 当小兵可被E击杀并且英雄身上有Buff", true).SetValue(true));
            菜单.SubMenu("Misc").AddItem(new MenuItem("EKS", "自动 E 击杀", true).SetValue(true));
            菜单.SubMenu("Misc").AddItem(new MenuItem("usePackets", "使用 封包", true).SetValue(false));
         ...
         /*   菜单.SubMenu("Drawing").AddItem(new MenuItem("minBRange", "Balista Min Range").SetValue(new Circle(false, Color.Chartreuse)));
            ...
            菜单.SubMenu("Message").AddItem(new MenuItem("Version", "版本 : 1.0.0.3"));
            菜单.SubMenu("Message").AddItem(new MenuItem("Version1", "对外QQ群 : 299606556"));
