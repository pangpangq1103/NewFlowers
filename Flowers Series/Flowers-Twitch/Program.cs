                    ...
                    if (Menu.Item("ComboE", true).GetValue<bool>())
                    {
                        if (E.IsReady())
                        {
                            if (e.IsValidTarget(E.Range))
                            {
                                ...
                                if (Me.ServerPosition.Distance(e.ServerPosition) >= E.Range * 0.7 && e.IsValidTarget(E.Range) && GetEStackCount(e) >= Menu.Item("MinComboEStack", true).GetValue<Slider>().Value)
                                {
                                    E.Cast();
                                }
                                else if (Menu.Item("UseEIfFullStack", true).GetValue<bool>() && GetEStackCount(e) >= 6)
                                {
                                    E.Cast();
                                }
                            }
