...
namespace Jungle_Timer_And_TurretDrawing
{
    public class JungleTimer
    {
        ...
        private static SharpDX.Direct3D9.Font _mapFont;
        private static SharpDX.Direct3D9.Font _miniMapFont;
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (JungleCamp jungleCamp in _jungleCamps.Where(camp => camp.NextRespawnTime > 0))
            {
                ...
                if (timeClock >= 150)
                {
                    Huabian.DrawText(_mapFont, time, (int)pos.X, (int)pos.Y - 8, SharpDX.Color.White);
                }
                if (timeClock < 150 && timeClock >= 100)
                {
                    Huabian.DrawText(_mapFont, time, (int)pos.X, (int)pos.Y - 8, SharpDX.Color.LimeGreen);
                }
                if (timeClock < 100 && timeClock >= 50)
                {
                    Huabian.DrawText(_mapFont, time, (int)pos.X, (int)pos.Y - 8, SharpDX.Color.Orange);
                }
                if (timeClock < 50)
                {
                    Huabian.DrawText(_mapFont, time, (int)pos.X, (int)pos.Y - 8, SharpDX.Color.Red);
                }
            }
        }
        ...
        public static bool 大地图()
        {
            return Flowers.菜单.Item("Timer").GetValue<bool>();
        }
        public static bool 小地图()
        {
            return Flowers.菜单.Item("Timer1").GetValue<bool>();
        }
